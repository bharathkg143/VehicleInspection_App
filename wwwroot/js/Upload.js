...
function clearImage() {
    // Remove the captured image element from the top div
    if (capturedImageElement) {
        capturedImageElement.parentNode.removeChild(capturedImageElement);
...
document.getElementById('upload-capture').addEventListener('click', async function () {
    if (capturedImageElement) {
        ...
        if (curLat !== null && curLat !== "" && curLat !== undefined && curLong !== null && curLong !== "" && curLong !== undefined) {
            ...
            //calling async function to get address based on lat log
            address = await fetchAddress(curLat, curLong);
        }
        else {
            lati = "Latitude: 0.0";
            longi = "Longitude: 0.0";
        ...
        ctx.strokeText(dateString, textX, height - 65);
        ctx.fillText(dateString, textX, height - 65);
        ctx.strokeText(address, textX, height - 45);
        ctx.fillText(address, textX, height - 45);
        ...
        canvas.toBlob(function (blob) {
            // Create a FormData object to send the image data to server
            ...
            fetch(rootPath + '/Upload/UploadVehicleDocuments', {
                method: 'POST',
                body: formData
            })
...
//Sending reuqest to api to fetch address
async function fetchAddress(latitude, longitude) {
    var loc;

    if (latitude && longitude) {
        const apiKey = '49d7ba44016647b298f0cb31b435ce7e';
        const apiUrl = `https://api.opencagedata.com/geocode/v1/json?q=${latitude},${longitude}&key=${apiKey}`;

        try {
            const response = await fetch(apiUrl);
            const data = await response.json();
            console.log('API Response after request:', data);
